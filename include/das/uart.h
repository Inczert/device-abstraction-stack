 ...
 *
 * Only the configured application data bits are transmitted. Any hardware
 * parity position is generated by the backend/device and is not supplied by
 * the caller.
 */
 ...
 *
 * Returned bytes contain only the configured application data bits; parity
 * positions used by the underlying UART hardware are not exposed.
 */
 ...
 * Only configured application data bits are placed on the UART data field.
 */
 ...
 * framing/parity/noise/overrun faults return DAS_ERROR_IO. Returned bytes
 * contain only application data bits and exclude any parity position.
 */
